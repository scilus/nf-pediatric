...
def get_data(layout, nSub, dwis, t1s, t2s, fs, default_readout, clean):
    """ Return subject data
    ...
    if fs:
        ...
        if len(t1_nSess) == 1:
            ...
            logging.warning('No T1w file found.')
        else:
    ...
    t2_path = ''
    t2_nSess = []
    if not t2s and clean:
        return {}

    for t2 in t2s:
        if 'session' in t2.entities:
            if t2.entities['session'] == nSess:
                t2_nSess.append(t2)
        else:
            t2_nSess.append(t2)

    if len(t2_nSess) == 1:
        t2_path = t2_nSess[0].path
    elif len(t2_nSess) == 0:
        logging.warning('No T2w file found.')
    else:
        t2_paths = [curr_t2.path for curr_t2 in t2_nSess]
        logging.warning('More than one T2 file found.'
                        ' [{}]'.format(','.join(t2_paths)))

    # Parse the participants.tsv file
    # Get the age of the participant
    age = ""
    try:
        os.path.exists(layout.get(extension='tsv', suffix='participants')[0])
        logging.info("Found participants.tsv file, extracting age.")
        metadata = pd.read_csv(
            layout.get(extension='tsv', suffix='participants')[0],
            sep='\t')
        ...
        for dwi in dwis:
            ...
            if args.fs:
                ...
                t1s = layout.get(subject=nSub,
                                 ...
                                 suffix='T1w')
                if t1s:
                    logging.info("Found {} T1w files".format(len(t1s)))
                t2s = layout.get(subject=nSub,
                                 datatype='anat', extension='nii.gz',
                                 suffix='T2w')
                if t2s:
                    logging.info("Found {} T2w files".format(len(t2s)))

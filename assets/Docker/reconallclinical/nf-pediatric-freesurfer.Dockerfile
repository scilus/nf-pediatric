...
RUN if [ "$TARGETARCH" = "arm64" ]; then \
    ...
    # Install Zlib
    # ----------------------------------------------------------------
RUN wget https://zlib.net/zlib-1.3.1.tar.gz -O zlib-1.3.1.tar.gz \
    ...
    # ----------------------------------------------------------------
RUN mkdir -p /opt/freesurfer-pkg \
    ...
    # ----------------------------------------------------------------
RUN git clone --branch dev https://github.com/freesurfer/samseg.git /stage/samseg \
    && cd /stage/samseg \
    ...
    # ----------------------------------------------------------------
RUN rm -rf /opt/freesurfer-pkg/packages \
    && echo "Cloning..." \
...
RUN cd / \
    ...
    # Install python reqs
    # ----------------------------------------------------------------
RUN sed -i 's/torch==2\.1\.2+cpu/torch==2.1.2/' /stage/freesurfer/freesurfer-dev/python/requirements-build.txt \
    && pip3 install -r /stage/freesurfer/freesurfer-dev/python/requirements-build.txt
    # Setup annex remote
    # ----------------------------------------------------------------
RUN cd /stage/freesurfer/freesurfer-dev \
...
#COPY ./compress_fs.sh /compress_fs.sh
#RUN chmod +x /compress_fs.sh \
#    && /compress_fs.sh \
#    && rm -f /compress_fs.sh
    # Remove unnecessary files for a recon-all-clinical.sh run.
RUN rm -rf /stage \
    && rm -rf \
...
# Copy the installed FreeSurfer from the build stage.
COPY --from=build /opt/freesurfer-dev /opt/freesurfer-dev
# Python deps.
